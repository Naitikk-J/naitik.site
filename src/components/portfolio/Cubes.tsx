...
const Cubes = ({
    ...
    const tiltAt = useCallback(
        (rowCenter: number, colCenter: number) => {
            ...
            const newActiveCubes = new Set<HTMLElement>();
            ...
            cubesRef.current.forEach(el => {
                ...
                if (dist <= radius) {
                    ...
                    newActiveCubes.add(el);
                } else if (activeCubesRef.current.has(el)) {
                    // Only reset if it was previously active
                    gsap.to(el, {
                        duration: leaveDur,
                        ease: 'power3.out',
                        overwrite: true,
            ...
            activeCubesRef.current = newActiveCubes;
        },
    ...
    const resetAll = useCallback(() => {
        ...
        activeCubesRef.current.forEach(el =>
            gsap.to(el, {
                duration: leaveDur,
                rotateX: 0,
                rotateY: 0,
    ...
    useEffect(() => {
        if (!sceneRef.current || !enableInteractions) return;
    ...
    }, [enableInteractions, onPointerMove, resetAll, onClick, onTouchMove, onTouchStart, onTouchEnd]);
